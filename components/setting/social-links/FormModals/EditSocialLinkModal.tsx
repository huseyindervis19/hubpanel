...
const EditSocialLinkModal: React.FC<Props> = ({ isOpen, link, onClose, onSuccess }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setMessage(messages["updated_successfully"] || "Link updated successfully!");
      setTimeout(() => {
        onClose();
        onSuccess();
      }, 1500);
      ...
      <div className="relative w-full h-[70vh] flex flex-col rounded-3xl bg-white dark:bg-gray-900">
        <div className="text-center px-4 pt-5">
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 mb-4">
            {messages["social_link_modal_description"] || "Edit your social platform."}
          </p>
        </div>
          ...
          <div className="flex-1 overflow-y-auto px-4 space-y-6 pb-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="space-y-2">
                ...
                <Input type="number" min={1} value={form.order} onChange={e => handleChange("order", e.target.value)} />
              </div>
            </div>
            ...
            <div className="space-y-2">
              ...
              <IconPicker value={form.icon} onChange={(iconName: IconName) => handleChange("icon", iconName)} />
            </div>
          </div>
