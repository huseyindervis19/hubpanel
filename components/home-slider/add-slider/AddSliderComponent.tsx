...
const AddSliderComponent: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setTimeout(() => {
        setMessage(null);
        router.push("/home-slider/list-slider");
      }, 1400);
    } catch (err) {
      console.error(err);
  ...
  return (
    <>
      <TitleComponent title={messages["add_home_slider"] || "Add Home Slider"} className="mb-6 font-semibold" />
        ...
        <Form onSubmit={handleSubmit} className="flex flex-col gap-4">
          ...
          <div className="mt-6 flex justify-end gap-3">
            <Button
              ...
              className={creating ? "opacity-75 cursor-not-allowed flex items-center justify-center" : "text-white"}
            >
              {creating ? (
                <>
                  <LoadingIcon
...
export default AddSliderComponent;
